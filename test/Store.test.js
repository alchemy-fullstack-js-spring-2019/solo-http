...
describe('Store', () => {
  ...
  it('creates an object in my store', () => {
    return store.create({ name: 'laura' })
      .then(createdPerson => {
        expect(createdPerson).toEqual({ name: 'laura', _id: expect.any(String) });
      });
  });
  ...
  it('updates an existing object', () => {
    return store.create({ name: 'larua' })
      .then(createdItem => {
      ...
      .then(updatedItem => {
        expect(updatedItem.name).toEqual('laura');
      });
  });
});
