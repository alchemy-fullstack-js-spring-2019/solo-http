...
class Store {
  ...
  findById(_id, callback) {
    fsPromises.readFile(this.storedFilePath(_id), { encoding: 'utf8' }, (err, data) => {
      try {
  ...
  find(callback) {
    fsPromises.readdir(this.rootDir, (err, listOfIds) => {
  ...
  findByIdAndDelete(_id, callback) {
    fsPromises.unlink(this.storedFilePath(_id), err => {
